import gamepy
import gamepy.surfarray as surfarray
import numpy as np
...
class BoolField:
    ...
    def find(self, pattern):  -> list(tuple(int))
        matches = []
        self_rows, self_cols = self.array.shape
        pattern_rows, pattern_cols = other.array.shape

        for i in range(self_rows - pattern_rows + 1):
            for j in range(self_cols - pattern_cols + 1)
                if np.array_equal(self.array[i:i+pattern_rows, j:j+pattern_cols])
                    mathes.append((i,  j))
...
def is_char(charfield, match_char):
